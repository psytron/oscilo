...
while True:
    ...
    drifter['x'] = drifter['x'] - (  ( drifter['x']-x) /2  )
    drifter['y'] = drifter['y'] - (  ( drifter['y']-y) /2  )    
    ...
    buffer = np.concatenate((buffer, sound_array))


    # If the buffer is full, process it using Pedalboard
    if buffer.size >= BUFFER_SIZE:
        processed_sound = pb.process(buffer, sample_rate=fs)
